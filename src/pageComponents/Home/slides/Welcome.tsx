...
export const WelcomeSlide: FC<SlideProps> = ({ isActive }) => {
  ...
  // Check if we should show the fight button (until end of next Thursday)
  useEffect(() => {
    const now = new Date()
    const endDate = new Date('2025-07-03T23:59:59') // End of next Thursday (July 3, 2025)

    setShowFightButton(now < endDate)
  }, [])
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Fight for $RODAI Banner - Top of page */}
      {showFightButton && (
        <motion.div
          ...
          className="w-full flex justify-center items-center mb-6 relative top-10"
        >
          <a target="_blank" href="https://fight.cryptofightclub.wtf/">
            <Button
              color="danger"
              className="text-white animate-pulse bg-red-600 hover:bg-red-700 text-xl px-12 py-6 font-bold rounded-xl shadow-lg transform hover:scale-105 transition-all duration-200"
              size="lg"
            >
              👊 Fight for $RODAI 👊
            </Button>
